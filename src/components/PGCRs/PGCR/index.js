...
class ReportItem extends React.Component {
  ...
  getProgression = async (membershipType, membershipId, characterId) => {
    ...
    const gloryPoints = characterProgressions[characterId].progressions[2000925172].currentProgress.toLocaleString(this.props.i18n.language);
    const valorPoints = characterProgressions[characterId].progressions[2626549951].currentProgress.toLocaleString(this.props.i18n.language);
    const infamyPoints = characterProgressions[characterId].progressions[2772425241].currentProgress.toLocaleString(this.props.i18n.language);
    ...
    const valorResets = valorSeasons.reduce((a, v) => a + v.resets, 0).toLocaleString(this.props.i18n.language);
    const infamyResets = infamySeasons.reduce((a, v) => a + v.resets, 0).toLocaleString(this.props.i18n.language);
          ...
          <li key={entry.characterId} className={cx('linked', { isExpandedPlayer })} onClick={() => this.togglePlayerHandler(entry.characterId)}>
            <div className={cx('inline', { dnf: dnf })}>
              ...
              {displayStats.header.map((s, i) => {
                ...
                if (s.extended) {
                  value = s.round ? Number.parseFloat(entry.extended.values[s.key].basic[s.type]).toLocaleString(this.props.i18n.language, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : entry.extended.values[s.key].basic[s.type];
                } else if (s.async) {
                  ...
                  value = s.round ? Number.parseFloat(entry[s.key].basic[s.type]).toLocaleString(this.props.i18n.language, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : entry[s.key].basic[s.type];
                } else {
                  value = s.round ? Number.parseFloat(entry.values[s.key].basic[s.type]).toLocaleString(this.props.i18n.language, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : entry.values[s.key].basic[s.type];
                }
                ...
                return (
                  <div key={h} className={cx('group', g.key)}>
                    ...
                    {g.fields.map((s, i) => {
                      let value;
                      if (s.extended) {
                        value = s.round ? Number.parseFloat(entry.extended.values[s.key].basic[s.type]).toLocaleString(this.props.i18n.language, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : entry.extended.values[s.key].basic[s.type].toLocaleString(this.props.i18n.language);
                      } else if (s.async) {
                        const cache = playerCache.find(p => p.membershipId === entry.player.destinyUserInfo.membershipId);
                        value = cache && cache[s.key] ? cache[s.key] : '–';
                        ...
                        value = s.round ? Number.parseFloat(entry.values[s.key].basic[s.type]).toLocaleString(this.props.i18n.language, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : entry.values[s.key].basic[s.type].toLocaleString(this.props.i18n.language);
                      }
