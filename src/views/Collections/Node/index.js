...
function PresentationNode({ settings, member, ...props }) {
  ...
  const primaryHash = props.match.params.primary;
  ...
  const secondaryHash =
    !props.match.params.secondary && definitionPrimary?.children.presentationNodes.find((child) => classNodes[character.classType].includes(child.presentationNodeHash))
      ...
      : !props.match.params.secondary || !manifest.DestinyPresentationNodeDefinition[props.match.params.secondary]
      ? // default to first node is no hash or can't find hash
        definitionPrimary?.children.presentationNodes[0].presentationNodeHash
      : props.match.params.secondary;

  const definitionSecondary = manifest.DestinyPresentationNodeDefinition[secondaryHash];

  const tertiaryHash =
    !props.match.params.tertiary && definitionSecondary?.children.presentationNodes.find((child) => classNodes[character.classType].includes(child.presentationNodeHash))
      ? // no node specified but found a class-specific node to default to
      ...
      : !props.match.params.tertiary || !manifest.DestinyPresentationNodeDefinition[props.match.params.tertiary]
      ? // default to first node is no hash or can't find hash
        definitionSecondary?.children.presentationNodes[0].presentationNodeHash
      : props.match.params.tertiary;

  const definitionTertiary = manifest.DestinyPresentationNodeDefinition[tertiaryHash];

  const quaternaryHash = props.match.params.quaternary ? props.match.params.quaternary : false;
  ...
  const secondaryChildren = definitionSecondary.children.presentationNodes
    ...
    .map((child) => {
      ...
      const secondaryTotal = settings.itemVisibility.hideInvisibleCollectibles ? states.filter((state) => !enums.enumerateCollectibleState(state).Invisible).length : states.length;
